    ...
    <div class="flex-1 flex flex-col">
        <header class="bg-white border-b border-slate-100 px-6 py-6 lg:px-10 lg:py-8 flex flex-wrap items-center justify-between sticky top-0 z-40 gap-4">
            <div class="flex items-center gap-4 w-full lg:w-auto">
                ...
                <div>
                    <h2 class="text-3xl font-black text-secondary tracking-tighter">Our <span class="text-primary">Team</span></h2>
                <p class="text-slate-400 text-xs font-black uppercase tracking-[4px] mt-1">People & Visionaries</p>
            </div>
            </div>
            <?php if ($action == 'list'): ?>
                <a href="team.php?action=add" class="bg-primary text-white px-8 py-3 rounded-[20px] font-black shadow-lg shadow-primary/20 hover:-translate-y-1 transition-all text-xs uppercase tracking-widest">Add Member</a>
            <?php else: ?>
                <a href="team.php" class="text-slate-400 font-black hover:text-secondary transition-colors text-xs uppercase tracking-widest"><i class="fas fa-arrow-left mr-2"></i> Back to List</a>
            <?php endif; ?>
        </header>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex items-center space-x-8 p-6 bg-slate-50 rounded-3xl border-2 border-dashed border-slate-200">
                                    <?php if (isset($editItem['image_path'])): ?>
                                        <img id="teamPreview" src="../<?php echo $editItem['image_path']; ?>" class="w-20 h-20 rounded-full object-cover shadow-md">
                                    <?php else: ?>
                                        <img id="teamPreview" src="../assets/img/user.jpg" class="w-20 h-20 rounded-full object-cover shadow-md">
                                    <?php endif; ?>
                                    <input type="file" name="image" onchange="openCropModal(this, document.getElementById('teamPreview'), 4/5)" class="text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-black file:bg-primary file:text-white hover:file:bg-primary/80 transition-all">
                                </div>
                            </div>
